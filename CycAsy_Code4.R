...
XTrainCycle<-cbind(A$SC[52:1344],A$CC[52:1344])

XTestCycle<-cbind(A$SC[1396:2256],A$CC[1396:2256])
...
InputTrain<-InputTrain[1:1293,2:49]

OutputTrain<-TrainSet[52:TrainSize]
...
InputTest<-InputTest[1:861,2:49]

OutputTest<-TestSet[52:TestSize]
...
Optimal_C<-c(10,100,1000,100,10,100,1000)

Optimal_eps<-c(0.0001,0,0.01,0.001,0,0,0.001)
...
write.csv(Results,"Model_Indexes4.csv")
write.csv(B,"Model_Errors4.csv")

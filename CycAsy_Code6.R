...
plot.frequency.spectrum <- function(X.k, xlimits=c(0,length(X.k))) {
  ...
  plot(plot.data, type = "h", col = "black", lwd = 5, main="", 
       xlab="Frequency (number of cycles)", ylab="Power", 
       xlim=xlimits, ylim=c(0,max(Mod(plot.data[,2]))))
}

a1=DataSet1$TOTALDEMAND
a2=DataSet2$TOTALDEMAND
...
XTrainCycle<-cbind(A$SC[54:1344],A$CC[54:1344])

XTestCycle<-cbind(A$SC[1398:2256],A$CC[1398:2256])
...
InputTrain<-InputTrain[1:1291,2:49]

OutputTrain<-TrainSet[54:TrainSize]
...
InputTest<-InputTest[1:859,2:49]

OutputTest<-TestSet[54:TestSize]
...
Optimal_C<-c(10,100,1000,10,100,10,1)

Optimal_eps<-c(0.0001,0.01,0.01,0.01,0.0001,0.001,0.001)
...
for (i in c(1:7)){
 ...
 # Prediction_Results<-cbind(OutputTest,Basic_LS_Prediction,Basic_LAV_Prediction,Basic_SVR_Prediction,Basic_MLP_Prediction,Asy_LAV_Prediction,Asy_SVR_Prediction)

Prediction_Results<-cbind(OutputTest,Basic_LS_Prediction,Basic_LAV_Prediction,Basic_SVR_Prediction,Asy_LAV_Prediction,Asy_SVR_Prediction)

  
  #Error_Results<-cbind(Basic_LS_Error,Basic_LAV_Error,Basic_SVR_Error,Basic_MLP_Error,Asy_LAV_Error, Asy_SVR_Error)

  Error_Results<-cbind(Basic_LS_Error,Basic_LAV_Error,Basic_SVR_Error,Asy_LAV_Error, Asy_SVR_Error)
  ...
  Ratio_Results<-rbind(LS_Ratio,LAV_Ratio,SVR_Ratio,Asy_LAV_Ratio,Asy_SVR_Ratio)

  #Cost_Results<-rbind(Basic_LS_Cost, Basic_LAV_Cost, Basic_SVR_Cost,Basic_MLP_Cost, Asy_LAV_Cost, Asy_SVR_Cost)
  Cost_Results<-rbind(Basic_LS_Cost, Basic_LAV_Cost, Basic_SVR_Cost, Asy_LAV_Cost, Asy_SVR_Cost)
  #Time_Results<-rbind(LS_time, LAV_time, SVR_time, MLP_time, AsyLAV_time,AsySVR_time)

  Time_Results<-rbind(LS_time, LAV_time, SVR_time, AsyLAV_time,AsySVR_time)
...
print(Results)
write.csv(Results,"Model_Indexes6.csv")
write.csv(B,"Model_Errors6.csv")

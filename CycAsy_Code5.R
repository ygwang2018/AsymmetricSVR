...
XTrainCycle<-cbind(A$SC[53:1344],A$CC[53:1344])

XTestCycle<-cbind(A$SC[1397:2256],A$CC[1397:2256])
...
InputTrain<-InputTrain[1:1292,2:49]

OutputTrain<-TrainSet[53:TrainSize]
...
InputTest<-InputTest[1:860,2:49]

OutputTest<-TestSet[53:TestSize]
...
Optimal_C<-c(100,1000,1,10,1000,100,1000)

Optimal_eps<-c(0.01,0.001,0.0001,0.1,0.01,0.001,0)
...
print(Results)
write.csv(Results,"Model_Indexes5.csv")
write.csv(B,"Model_Errors5.csv")

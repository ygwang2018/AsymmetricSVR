...
XTrainCycle<-cbind(A$SC[50:1344],A$CC[50:1344])

XTestCycle<-cbind(A$SC[1394:2256],A$CC[1394:2256])
...
InputTrain<-InputTrain[1:1295,2:49]

OutputTrain<-TrainSet[50:TrainSize]
...
InputTest<-InputTest[1:863,2:49]

OutputTest<-TestSet[50:TestSize]
...
Optimal_C<-c(10,10,100,100,1000,1000,1)

Optimal_eps<-c(0,0,0,0.001,0.0001,0.001,0.001)
...
write.csv(Results,"Model_Indexes2.csv")
write.csv(B,"Model_Errors2.csv")
